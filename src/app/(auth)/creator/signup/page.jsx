...
import CreatorVideoWallShowcase from "@/components/auth/CreatorVideoWallShowcase";
...
export default function CreatorSignupPage() {
  ...
  const handleSubmit = (e) => {
    ...
    const check = checkEmailAvailability(formData.email, "creator");
    if (!check.available) {
  ...
  const handleQuickRegister = () => {
    const demoEmail = "maya@creatorz.io";
    const check = checkEmailAvailability(demoEmail, "creator");
  ...
  return (
    <div className="relative min-h-[calc(100vh-5rem)] lg:h-[calc(100vh-5rem)] flex flex-col lg:flex-row bg-[#FAF3EB] lg:overflow-hidden selection:bg-[#66101B]/20 selection:text-[#520B15]">
      ...
      {/* Left Half: Creator Video Wall (Desktop: Left 50%, Mobile/Tablet: Below form) */}
        ...
        <div className="w-full max-w-[490px] lg:max-w-[510px] bg-[#FFFDF9]/95 backdrop-blur-2xl border border-[#DECDBE] rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-7 shadow-[0_12px_28px_rgba(77,10,19,0.06),0_0_0_1px_rgba(255,255,255,0.7)_inset] my-auto">
          ...
          <div className="mb-3.5">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/creator/login" className="text-xs font-semibold text-[#66101B] hover:text-[#4A0711] hover:underline transition-colors">
                Already registered?
              </Link>
            </div>
            ...
            <p className="text-xs sm:text-sm text-[#7D5358] font-serif mt-0.5">
              Set up your verified creator handle and pitch to open briefs.
            </p>
          </div>

          {/* Clean 2-Column Quick Actions Row (Saves space, declutters top) */}
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <div>
                ...
                <div className="relative">
                  <AtSign className="w-4 h-4 text-[#8C5D64] absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                  <input
                    type="text"
                    required
                    value={formData.handle}
                    onChange={(e) => setFormData({ ...formData, handle: e.target.value })}
                    placeholder="@maya_creates"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    {categories.map((c) => (
                      <option key={c} value={c} className="bg-white text-[#4A0711]">
                        {c}
                      </option>
                    ))}
                  </select>
            ...
            <div className="pt-0.5">
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <span className="text-[11px] text-[#7D5358] leading-tight">
                  I agree to CreatorZ Deliverable Standards & collaboration terms.
                </span>
              </label>
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-[#DECDBE] text-center space-y-1">
            <p className="text-xs text-[#7D5358]">
              ...
              <Link href="/creator/login" className="text-[#66101B] font-bold hover:text-[#4A0711] hover:underline transition-colors">
                Sign In to Creator Dashboard
              </Link>
            </p>
            <p className="text-[11px] text-[#7D5358]">
              Are you a brand commissioner?{" "}
              <Link href="/brand/signup" className="text-[#9E7B35] font-bold hover:text-[#7D6122] hover:underline transition-colors">
                Register as Brand
              </Link>
            </p>
          </div>
