...
import { Video, Upload, Check, ArrowLeft, Clock, ShieldCheck, Play, Pause } from "lucide-react";
import confetti from "canvas-confetti";

interface ApplyBriefProps {
  params: Promise<{ id: string }>;
}

export default function ApplyBriefPage({ params }: ApplyBriefProps) {
  const resolvedParams = use(params);
  const router = useRouter();
  const brief = MOCK_OPEN_BRIEFS.find((b) => b.id === resolvedParams.id) || MOCK_OPEN_BRIEFS[0];

  const [proposedRate, setProposedRate] = useState<number>(22000);
  const [deliveryDays, setDeliveryDays] = useState<number>(3);
  const [hookText, setHookText] = useState<string>("Stop drinking protein shakes that taste like chalk powder...");
  const [pitchSummary, setPitchSummary] = useState<string>("I will shoot a fast-paced kitchen comedy sketch showing instant cold water dissolution vs. clumpy competitor shakes.");
  const [videoFileUploaded, setVideoFileUploaded] = useState<boolean>(true);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#F0E8D8]">
              <div className="flex items-center gap-3.5">
                <img
                  src={brief.brandLogo}
                  alt={brief.brandName}
                  className="w-12 h-12 rounded-xl object-cover border border-[#E8DEC8]"
                />
                <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-5 space-y-3">
                ...
                <div className="relative aspect-[9/16] bg-black rounded-2xl overflow-hidden border-2 border-[#E8DEC8] shadow-md group">
                  <video
                    ref={videoRef}
                    src="https://assets.mixkit.co/videos/preview/mixkit-hands-holding-a-smartphone-with-green-screen-41541-large.mp4"
                    loop
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    className="flex-1 py-2 rounded-xl bg-[#FAF6EE] hover:bg-[#F2EAE0] border border-[#D8CEBD] text-[#181314] text-xs font-semibold flex items-center justify-center gap-1.5"
                  >
              ...
              <div className="lg:col-span-7 space-y-4 text-xs">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        min={1000}
                        step={1000}
                        className="w-full text-xs pl-7 pr-3 py-2.5 rounded-xl border border-[#E8DEC8] bg-[#FAF6EE] font-mono font-bold text-[#181314] focus:outline-none"
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min={1}
                      max={14}
                      className="w-full text-xs p-2.5 rounded-xl border border-[#E8DEC8] bg-[#FAF6EE] font-mono font-bold text-[#181314] focus:outline-none"
                      required
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Stop drinking protein shakes that taste like chalk powder..."
                    className="w-full text-xs p-3 rounded-xl border border-[#E8DEC8] bg-[#FAF6EE] text-[#181314] focus:outline-none focus:border-[#7A1C28]"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Outline how you will shoot the reel, the characters involved, product demonstration, and call to action..."
                    className="w-full text-xs p-3 rounded-xl border border-[#E8DEC8] bg-[#FAF6EE] text-[#181314] focus:outline-none focus:border-[#7A1C28]"
                    required
                  />
                </div>
                ...
                <div className="pt-4 border-t border-[#F0E8D8] flex justify-end">
                  <button
                    type="submit"
                    className="px-6 py-3 rounded-full bg-[#7A1C28] hover:bg-[#63141E] text-white font-bold text-xs shadow-md transition transform active:scale-95"
                  >
