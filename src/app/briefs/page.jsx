...
import { Briefcase, Video, Users, Clock, Plus } from "lucide-react";
export default function OpenBriefsPage() {
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-[#E8DEC8]">
        ...
        <Link href="/briefs/new" className="px-5 py-2.5 rounded-full bg-[#7A1C28] hover:bg-[#63141E] text-white text-xs font-bold shadow-sm transition flex items-center gap-2 self-start md:self-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {MOCK_OPEN_BRIEFS.map((brief) => (<div key={brief.id} className="bg-white border border-[#E8DEC8] rounded-2xl p-6 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              
              {/* Brand Header */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <img src={brief.brandLogo} alt={brief.brandName} className="w-10 h-10 rounded-xl object-cover border border-[#E8DEC8]"/>
                  <div>
            ...
            <div className="pt-4 border-t border-[#F0E8D8] flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Link href={`/campaigns/${brief.id}/applicants`} className="text-xs font-semibold text-[#6C635B] hover:text-[#181314] hover:underline">
                  Review Deck
                </Link>
                <Link href={`/briefs/${brief.id}/apply`} className="px-3.5 py-2 rounded-lg bg-[#7A1C28] hover:bg-[#63141E] text-white text-xs font-bold shadow-sm transition flex items-center gap-1">
