...
export default function OrderRoomPage() {
    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-8">
      ...
      {showApprovedSuccess && (<div className="bg-[#EBF7EE] border-2 border-[#16A34A] rounded-2xl p-6 shadow-md text-[#166534] space-y-3">
          ...
          <div className="flex gap-3 pt-2">
            <Link href={`/creators/${order.creatorUsername}`} className="text-xs font-bold px-4 py-2 rounded-lg bg-[#166534] text-white hover:bg-[#14532D]">
      ...
      <div className="bg-white border border-[#E8DEC8] rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
        ...
        <div className="relative">
          <div className="w-full bg-[#FAF6EE] h-2 rounded-full overflow-hidden border border-[#E8DEC8]">
            <div className="bg-[#7A1C28] h-full rounded-full transition-all duration-700" style={{ width: `${(order.currentMilestoneStage / 6) * 100}%` }}></div>
          </div>
        </div>

        {/* 6 Milestone Nodes */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {order.milestones.map((m) => (<div key={m.stage} className={`p-3.5 rounded-xl border text-xs space-y-1.5 transition-all ${m.status === "completed"
