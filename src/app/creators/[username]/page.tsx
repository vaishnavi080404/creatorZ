...
import { ShieldCheck, Eye, MessageSquare, Clock, MapPin, Globe, CheckCircle2, Send, Award, Sparkles } from "lucide-react";

interface CreatorPageProps {
  params: Promise<{ username: string }>;
}

export default function CreatorDossierPage({ params }: CreatorPageProps) {
      ...
      <div className="bg-white border border-[#E8DEC8] rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6 pb-6 border-b border-[#F0E8D8]">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            <img
              src={creator.avatar}
              alt={creator.name}
              className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl object-cover border-2 border-[#E8DEC8] shadow-sm flex-shrink-0"
            />
            ...
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                {creator.openToBrands && (
                  <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#EBF7EE] text-[#166534] text-xs font-semibold border border-[#C6E7CE]">
          ...
          <div className="bg-[#FAF6EE] border border-[#E8DEC8] rounded-2xl p-5 text-right min-w-[240px] space-y-3 self-stretch sm:self-auto flex flex-col justify-between">
            ...
            <Link
              href="/messages"
              className="w-full py-2.5 rounded-xl bg-[#7A1C28] hover:bg-[#63141E] text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-sm transition transform active:scale-95"
            >
        ...
        <div className="pt-2">
          ...
          <div className="flex flex-wrap gap-2">
            {creator.provenFormats.map((fmt, i) => (
              <span key={i} className="px-3 py-1 rounded-lg text-xs font-semibold bg-[#FAF6EE] text-[#181314] border border-[#E8DEC8]">
                {fmt}
              </span>
            ))}
          </div>
