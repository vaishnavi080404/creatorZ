    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-8">
      ...
      <div className="bg-white border border-[#E8DEC8] rounded-2xl p-4 shadow-sm flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="relative flex-1 w-full">
          ...
          <input type="text" placeholder="Search by creator name, niche, language (Hindi, Tamil, English)..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full text-xs pl-10 pr-4 py-2.5 rounded-xl bg-[#FAF6EE] border border-[#E8DEC8] text-[#181314] placeholder:text-[#6C635B] focus:outline-none focus:border-[#7A1C28]"/>
        </div>

        {/* Tier Filter Pills */}
        <div className="flex items-center gap-1.5 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          <span className="text-xs font-bold text-[#6C635B] mr-1 hidden sm:inline">Tier:</span>
          {tiers.map((tier) => (<button key={tier} onClick={() => setSelectedTier(tier)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${selectedTier === tier
        ...
        <div className="w-full md:w-auto">
          <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full text-xs px-3 py-2.5 rounded-xl bg-[#FAF6EE] border border-[#E8DEC8] text-[#181314] font-semibold focus:outline-none cursor-pointer">
      ...
      {/* Creator Grid */}
      {filteredCreators.length === 0 ? (<div className="bg-white border border-[#E8DEC8] rounded-2xl p-12 text-center">
