...
export default function CreatorCard({ creator }) {
    return (<div className="bg-white border border-[#E8DEC8] rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col justify-between">
      
      <div>
        {/* Top Portrait Image & Video Preview Container */}
        <div className="relative aspect-[4/5] bg-zinc-900 overflow-hidden">
          <img src={creator.avatar} alt={creator.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-95"/>
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
            ...
            {creator.openToBrands && (<span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/60 text-emerald-400 text-[10px] font-semibold backdrop-blur-md border border-emerald-500/30">
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            {creator.provenFormats.slice(0, 2).map((fmt, i) => (<span key={i} className="text-[10px] px-2 py-0.5 rounded bg-[#FAF6EE] text-[#6C635B] border border-[#E8DEC8]">
                {fmt}
              </span>))}
          </div>
        </div>
      ...
      <div className="p-5 pt-0">
        <div className="pt-3 border-t border-[#F0E8D8] flex items-center justify-between">
          ...
          <Link href={`/creators/${creator.username}`} className="text-xs font-bold px-4 py-2 rounded-lg bg-[#FAF6EE] hover:bg-[#7A1C28] text-[#7A1C28] hover:text-white border border-[#E8DEC8] hover:border-[#7A1C28] transition-all flex items-center gap-1">
            <span>Dossier</span>
