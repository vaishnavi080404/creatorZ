...
export default function CreatorOperatingSystem() {
    ...
    return (<section className="relative bg-[#faf3eb] pt-24 pb-28 sm:pt-32 sm:pb-36 border-b border-[#e4d0c0] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16 relative z-10">
        ...
        <div className="text-center max-w-4xl mx-auto space-y-5">
          ...
          {/* Eyebrow Pill */}
          <motion.div initial={{ opacity: 0.001, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ delay: 0.1, duration: 0.6, ease: FRAMER_EASE }} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#f3e8dc] border border-[#e4d0c0] text-[#66101b] text-xs font-mono font-bold tracking-wider uppercase shadow-2xs">
            <span className="w-1.5 h-1.5 rounded-full bg-[#66101b]"/>
            <span>TALENT & BRAND OPERATING SYSTEM</span>
          </motion.div>

          {/* Main Headline */}
          <motion.h2 initial={{ filter: "blur(8px)", opacity: 0.001, y: 30 }} whileInView={{ filter: "blur(0px)", opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ delay: 0.2, duration: 0.65, ease: FRAMER_EASE }} className="text-4xl sm:text-6xl lg:text-[70px] font-bold font-heading text-black leading-[1.08] tracking-tight">
            One place for proof, <br />
            reach, context, and collaboration.
          </motion.h2>

          {/* Subtitle */}
          <motion.p initial={{ filter: "blur(4px)", opacity: 0.001, y: 20 }} whileInView={{ filter: "blur(0px)", opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ delay: 0.3, duration: 0.6, ease: FRAMER_EASE }} className="text-base sm:text-lg text-[#82575c] leading-relaxed max-w-2xl mx-auto font-normal">
          ...
          <motion.div initial={{ opacity: 0.001, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ delay: 0.4, duration: 0.6, ease: FRAMER_EASE }} className="flex flex-wrap items-center justify-center gap-4 pt-1">
            <Link href="/brand/briefs/new" className="px-8 py-3.5 rounded-full bg-[#66101b] hover:bg-[#4d0a13] text-[#faf3eb] text-sm font-semibold transition shadow-md flex items-center gap-2 group">
              <span>Post a Campaign Brief</span>
              <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1"/>
            </Link>

            <Link href="/creators" className="px-8 py-3.5 rounded-full bg-white hover:bg-[#faf3eb] border-2 border-[#e4d0c0] text-black text-sm font-semibold transition shadow-xs hover:border-[#66101b]">
      ...
      <div className="relative w-full my-12 sm:my-16 h-[580px] sm:h-[630px] overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex items-center overflow-hidden w-full select-none [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)] pointer-events-none">
          ...
          <div className="flex shrink-0 items-center gap-5 animate-marquee will-change-transform">
            {tickerVideos.map((video, idx) => (<div key={`block1-${video.id}-${idx}`} className="w-[260px] sm:w-[290px] h-[320px] sm:h-[355px] rounded-[28px] overflow-hidden shrink-0 bg-neutral-900 shadow-md border-2 border-[#e4d0c0]/80 relative">
                <video src={video.src} autoPlay loop muted playsInline className="w-full h-full object-cover brightness-100 contrast-105"/>
              </div>))}
          </div>
          ...
          <div className="flex shrink-0 items-center gap-5 animate-marquee pl-5 will-change-transform" aria-hidden="true">
            {tickerVideos.map((video, idx) => (<div key={`block2-${video.id}-${idx}`} className="w-[260px] sm:w-[290px] h-[320px] sm:h-[355px] rounded-[28px] overflow-hidden shrink-0 bg-neutral-900 shadow-md border-2 border-[#e4d0c0]/80 relative">
                <video src={video.src} autoPlay loop muted playsInline className="w-full h-full object-cover brightness-100 contrast-105"/>
              </div>))}
          </div>
        ...
        <div className="relative z-20 flex items-center justify-center pointer-events-auto">
          <motion.div initial={{ opacity: 0.001, scale: 0.94 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: FRAMER_EASE }} className="relative w-[280px] sm:w-[302px] h-[570px] sm:h-[614px]">
            {/* Real Full-Color Creator Video Playing Inside Phone */}
            <div className="absolute inset-[10px] overflow-hidden bg-black" style={{ borderRadius: "44px" }}>
              <video src="/impact-d6.mp4" autoPlay loop muted playsInline className="w-full h-full object-cover object-center"/>
            </div>

            {/* iPhone Lock Screen UI Overlay (Dynamic Island, Clock 9:41, Camera & Flashlight icons) */}
            <img src="/iphone-ui-overlay.png" alt="" className="absolute inset-[8px] z-10 w-[calc(100%-16px)] h-[calc(100%-16px)] object-contain pointer-events-none"/>

            {/* Real Transparent iPhone Titanium Hardware Frame (Crisp with zero dark shadow bleeds) */}
            <img src="/iphone-frame-transparent.png" alt="iPhone Device Frame" className="absolute inset-0 z-20 w-full h-full object-contain pointer-events-none"/>
          </motion.div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-16 space-y-16 relative z-10">
        ...
        ].map((cat) => (<span key={cat.id} className="px-4 py-2 rounded-full text-xs font-semibold flex items-center gap-1.5 cursor-default bg-white border border-[#e4d0c0] text-[#66101b] shadow-2xs hover:border-[#66101b] transition-colors">
                <span>{cat.icon}</span>
                <span>{cat.label}</span>
              </span>))}
          </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 pb-4 pt-6 border-t border-[#e4d0c0]">
          ...
          <div className="relative w-full max-w-[390px] h-[190px] sm:h-[210px] self-center lg:self-auto">
            ...
            <motion.div initial={{ opacity: 0.001, x: 25, y: 15 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.6, ease: FRAMER_EASE }} className="absolute right-0 top-3 sm:top-1 z-10 w-[260px] sm:w-[280px] bg-white rounded-3xl p-5 shadow-card-elevated border-2 border-[#e4d0c0] space-y-3">
            ...
            <motion.div initial={{ opacity: 0.001, x: -20, y: -10 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.35, duration: 0.6, ease: FRAMER_EASE }} className="absolute left-0 top-0 z-20 w-[200px] sm:w-[220px] bg-white rounded-3xl p-4 sm:p-5 shadow-2xl border-2 border-[#e4d0c0] space-y-3">
              ...
              <div className="space-y-2">
                <div onClick={() => toggleSetting("mediaKit")} className="flex items-center justify-between cursor-pointer group">
                ...
                <div onClick={() => toggleSetting("collaboration")} className="flex items-center justify-between cursor-pointer group">
                  <span className="text-xs font-medium text-black group-hover:text-[#66101b] transition-colors">
                    30s Audition Reels
                  </span>
                ...
                <div onClick={() => toggleSetting("payoutLedger")} className="flex items-center justify-between cursor-pointer group">
                  <span className="text-xs font-medium text-black group-hover:text-[#66101b] transition-colors">
                    Milestone Approvals
                  </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ delay: 0.15, duration: 0.6, ease: FRAMER_EASE }} whileHover={{ y: -6, transition: { duration: 0.2, ease: "easeOut" } }} className="relative rounded-3xl overflow-hidden min-h-[340px] sm:min-h-[380px] p-7 sm:p-8 flex flex-col justify-between shadow-xs border-2 border-[#e4d0c0] hover:border-[#66101b] hover:shadow-card-elevated transition-all duration-300 group cursor-default">
            <div className="absolute inset-0 z-0">
              <img src="/theme-comedy-artist.jpg" alt="Multi-Discipline Artists" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 brightness-95"/>
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ delay: 0.3, duration: 0.6, ease: FRAMER_EASE }} whileHover={{ y: -6, transition: { duration: 0.2, ease: "easeOut" } }} className="bg-white rounded-3xl p-7 sm:p-8 flex flex-col justify-between min-h-[340px] sm:min-h-[380px] shadow-xs border-2 border-[#e4d0c0] hover:border-[#66101b] hover:shadow-card-elevated transition-all duration-300 group cursor-default">
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ delay: 0.45, duration: 0.6, ease: FRAMER_EASE }} whileHover={{ y: -6, transition: { duration: 0.2, ease: "easeOut" } }} className="bg-white rounded-3xl p-7 sm:p-8 flex flex-col justify-between min-h-[340px] sm:min-h-[380px] shadow-xs border-2 border-[#e4d0c0] hover:border-[#66101b] hover:shadow-card-elevated transition-all duration-300 group cursor-default">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-bold tracking-widest text-[#66101b] uppercase">
                CONTRACTS & TRUST
              </span>
