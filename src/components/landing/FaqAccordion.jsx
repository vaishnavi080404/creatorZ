...
export default function FaqAccordion() {
    ...
    return (<section className="bg-[#f3e8dc] border-t border-[#e4d0c0] py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-12">
        ...
        <div className="space-y-4">
          {faqs.map((faq, i) => {
            ...
            return (<div key={i} className="bg-white/95 border-2 border-[#e4d0c0] rounded-2xl overflow-hidden shadow-xs hover:border-[#66101b] transition-all">
                <button type="button" onClick={() => setOpenIndex(isOpen ? null : i)} className="w-full p-6 text-left flex items-center justify-between gap-4 font-bold text-sm sm:text-base text-[#66101b] font-heading cursor-pointer">
                  <span className="leading-snug">{faq.q}</span>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-transform ${isOpen ? "bg-[#66101b] text-[#faf3eb] rotate-180" : "bg-[#faf3eb] text-[#66101b]"}`}>
                    <ChevronDown className="w-4 h-4"/>
                  </div>
                </button>

                {isOpen && (<div className="px-6 pb-6 pt-1 text-xs sm:text-sm text-[#82575c] leading-relaxed border-t border-[#e4d0c0] mt-1">
                    {faq.a}
                  </div>)}
              </div>);
        })}
