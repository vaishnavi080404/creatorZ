...
export default function ProblemSolution() {
  ...
  const slides = [
    {
      ...
      subtitle:
        "Brands find themselves locked in siloed categorization models, missing out on actors who sing, dancers who host, or stand-up comics who create brand content. Top talent stays hidden behind agency gatekeepers and basic search filters.",
      statNumber: "0%",
    ...
    },
    {
      ...
      subtitle:
        "Traditional talent managers charge 20–40% agency commissions while brand marketing teams wait weeks for pitch decks and quotes. Collaboration stays slow, expensive, and dry.",
      statNumber: "24h",
    ...
    },
    {
      ...
      title2: "Direct deals.",
      subtitle:
        "Brands post campaign briefs and receive 30-second audition reels from screen actors, comedians, singers, and digital creators within 24 hours. Structured milestone approvals ensure 100% deal certainty.",
      statNumber: "100%",
  ...
  return (
    <section className="relative bg-[#faf3eb] border-y border-[#e4d0c0] py-20 sm:py-28 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="pt-2">
              <div className="flex items-center bg-white rounded-full p-1.5 pl-5 max-w-md shadow-md border-2 border-[#e4d0c0] transition focus-within:border-[#66101b]">
                <input
                  ...
                  disabled={submitted}
                  required
                  className="bg-transparent text-black placeholder:text-[#82575c]/60 text-sm outline-none flex-1 pr-2 font-sans"
                />
                <button
                  type="submit"
                  className="bg-[#66101b] text-[#faf3eb] px-6 py-3 rounded-full text-xs font-bold hover:bg-[#4d0a13] active:scale-95 transition cursor-pointer tracking-wider shrink-0 shadow-sm"
                >
              ...
              {slides.map((slide, idx) => {
                ...
                return (
                  <button
                    ...
                    className="relative w-1.5 h-16 rounded-full bg-[#e4d0c0] overflow-hidden cursor-pointer hover:bg-[#eddcd0] transition"
                  >
                    {isActive ? (
                      <div
                        className="w-full bg-[#66101b] rounded-full origin-top"
                        style={{ height: `${progress}%` }}
                      />
                    ) : isPast ? (
                      <div className="w-full h-full bg-[#66101b]/60 rounded-full" />
                    ) : (
            ...
            {slides.map((_, idx) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all cursor-pointer ${
                  activeIndex === idx ? "w-8 bg-[#66101b]" : "w-2 bg-[#e4d0c0]"
                }`}
