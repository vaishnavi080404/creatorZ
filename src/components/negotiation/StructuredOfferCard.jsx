...
export default function StructuredOfferCard({ offer, onAccept, onCounter, onDecline }) {
    const [currentOffer, setCurrentOffer] = useState(offer);
    const [showCounterModal, setShowCounterModal] = useState(false);
    const [counterRate, setCounterRate] = useState(offer.baseAmount + 5000);
    ...
    const handleAccept = () => {
        ...
        setCurrentOffer(Object.assign(Object.assign({}, currentOffer), { status: "accepted" }));
    ...
    const handleCounterSubmit = (e) => {
        ...
        setCurrentOffer(Object.assign(Object.assign({}, currentOffer), { baseAmount: counterRate, gstAmount: newGst, totalAmount: counterRate + newGst, status: "countered" }));
        setShowCounterModal(false);
        setStatusMessage(`Counter Proposal of ₹${counterRate.toLocaleString()} submitted to brand.`);
        if (onCounter)
            onCounter(counterRate);
    };
    const handleDecline = () => {
        setCurrentOffer(Object.assign(Object.assign({}, currentOffer), { status: "declined" }));
        setStatusMessage("Offer safely declined.");
        if (onDecline)
            onDecline();
    };
    return (<div className="bg-white border border-[#E8DEC8] rounded-2xl p-6 shadow-sm max-w-xl mx-auto">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-[#F0E8D8] mb-4">
        ...
        <span className={`px-2.5 py-0.5 rounded text-[11px] font-mono font-bold border ${currentOffer.status === "accepted"
      ...
      {statusMessage && (<div className="mb-4 p-3 rounded-xl bg-[#FAF6EE] border border-[#E8DEC8] text-xs font-semibold text-[#7A1C28] text-center">
          {statusMessage}
        </div>)}
      ...
      <div className="space-y-4 text-xs mb-6">
        <div className="flex items-center gap-3">
          <img src={currentOffer.brandLogo} alt={currentOffer.brandName} className="w-10 h-10 rounded-lg object-cover border border-[#E8DEC8]"/>
          <div>
        ...
        <div className="p-3.5 rounded-xl bg-[#FAF6EE]/70 border border-[#E8DEC8] space-y-1.5">
          ...
          <ul className="space-y-1 text-[11px] text-[#6C635B]">
            {currentOffer.contractTerms.map((term, i) => (<li key={i} className="flex items-start gap-1.5">
      ...
      {/* Action Buttons */}
      {currentOffer.status === "pending" && (<div className="grid grid-cols-3 gap-3 pt-2">
          
          {/* ACCEPT & CONFIRM DEAL */}
          <button onClick={handleAccept} className="py-3 rounded-xl bg-[#7A1C28] hover:bg-[#63141E] text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-sm transition transform active:scale-95">
            <Check className="w-4 h-4"/>
            <span>Accept Offer</span>
          </button>

          {/* COUNTER RATE */}
          <button onClick={() => setShowCounterModal(true)} className="py-3 rounded-xl bg-[#FAF6EE] hover:bg-[#F2EAE0] border border-[#D8CEBD] text-[#181314] font-bold text-xs flex items-center justify-center gap-1.5 transition">
            <RefreshCw className="w-3.5 h-3.5 text-[#7A1C28]"/>
            <span>Counter Rate</span>
          </button>

          {/* DECLINE */}
          <button onClick={handleDecline} className="py-3 rounded-xl bg-white hover:bg-rose-50 border border-rose-200 text-rose-700 font-bold text-xs flex items-center justify-center gap-1.5 transition">
      ...
      {/* Counter Offer Modal */}
      {showCounterModal && (<div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            <form onSubmit={handleCounterSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={counterRate} onChange={(e) => setCounterRate(Number(e.target.value))} min={1000} step={1000} className="w-full text-sm pl-7 pr-3 py-2 border border-[#E8DEC8] rounded-lg font-mono font-bold text-[#181314] focus:outline-none" required/>
                </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setShowCounterModal(false)} className="px-4 py-2 rounded-lg border border-[#D8CEBD] text-xs font-semibold text-[#6C635B]">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 rounded-lg bg-[#7A1C28] text-white text-xs font-bold hover:bg-[#63141E]">
