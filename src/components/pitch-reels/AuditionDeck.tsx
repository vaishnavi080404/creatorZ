...
import { PitchReel, StructuredOffer } from "@/lib/types";
import { Check, X, Play, Pause, Volume2, VolumeX, ShieldCheck, Clock, Eye, Sparkles, Send } from "lucide-react";
import confetti from "canvas-confetti";
import Link from "next/link";

interface AuditionDeckProps {
  pitchReels: PitchReel[];
  campaignTitle: string;
}

export default function AuditionDeck({ pitchReels, campaignTitle }: AuditionDeckProps) {
  const [currentIndex, setCurrentIndex] = useState<number>(0);
  const [isPlaying, setIsPlaying] = useState<boolean>(true);
  const [isMuted, setIsMuted] = useState<boolean>(true);
  const [shortlisted, setShortlisted] = useState<PitchReel[]>([]);
  const [passed, setPassed] = useState<PitchReel[]>([]);
  const [showOfferModal, setShowOfferModal] = useState<boolean>(false);
  const [selectedForOffer, setSelectedForOffer] = useState<PitchReel | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (showOfferModal || isFinished) return;
      if (e.key === "ArrowLeft") handlePass();
      if (e.key === "ArrowRight") handleShortlist();
      if (e.key === " ") {
        e.preventDefault();
        togglePlay();
      }
  ...
  if (isFinished) {
    return (
      <div className="bg-white border border-[#E8DEC8] rounded-2xl p-12 text-center max-w-2xl mx-auto shadow-sm">
        ...
        <div className="flex justify-center gap-3">
          ...
          <Link
            href="/messages"
            className="text-xs font-bold px-5 py-2.5 rounded-full bg-[#7A1C28] text-white hover:bg-[#63141E] shadow-sm"
          >
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 bg-black rounded-2xl overflow-hidden relative aspect-[9/16] shadow-2xl border border-[#E8DEC8] group">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/40 flex flex-col justify-between p-4 pointer-events-none">
            ...
            <div className="flex items-center justify-between pointer-events-auto">
              ...
              <button
                onClick={toggleMute}
                className="w-8 h-8 rounded-full bg-black/60 text-white flex items-center justify-center backdrop-blur-md border border-white/20 hover:bg-black/80 transition"
              >
            ...
            <div className="space-y-2 pointer-events-auto">
              ...
              <div className="flex items-center justify-between pt-1">
                <button
                  onClick={togglePlay}
                  className="px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 text-white text-xs font-mono backdrop-blur-md flex items-center gap-1.5 transition"
                >
        ...
        <div className="lg:col-span-7 bg-white border border-[#E8DEC8] rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start justify-between pb-5 border-b border-[#F0E8D8]">
              <div className="flex items-center gap-4">
                <img
                  src={currentPitch.creatorAvatar}
                  alt={currentPitch.creatorName}
                  className="w-14 h-14 rounded-xl object-cover border border-[#E8DEC8]"
                />
                <div>
                  <div className="flex items-center gap-2">
          ...
          <div className="pt-6 border-t border-[#F0E8D8] grid grid-cols-2 gap-4">
            ...
            <button
              onClick={handlePass}
              className="py-3.5 rounded-xl border border-[#D8CEBD] bg-[#FAF6EE] hover:bg-[#F2EAE0] text-[#181314] font-bold text-xs flex items-center justify-center gap-2 transition transform active:scale-95 shadow-sm"
            >
            ...
            <button
              ...
              className="py-3.5 rounded-xl bg-[#7A1C28] hover:bg-[#63141E] text-white font-bold text-xs flex items-center justify-center gap-2 transition transform active:scale-95 shadow-md"
            >
      ...
      {showOfferModal && selectedForOffer && (
              ...
              <div>
                <div className="flex items-center justify-between pb-4 border-b border-[#E8DEC8] mb-4">
                  ...
                  <button
                    ...
                    className="w-7 h-7 rounded-full bg-[#FAF6EE] text-[#181314] font-bold text-xs flex items-center justify-center hover:bg-[#F2EAE0]"
                  >
                    ✕
                  </button>
                </div>
                ...
                <div className="space-y-4 text-xs mb-6">
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-[#FAF6EE] border border-[#E8DEC8]">
                    <img
                      src={selectedForOffer.creatorAvatar}
                      alt={selectedForOffer.creatorName}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => setShowOfferModal(false)}
                    className="py-2.5 rounded-lg border border-[#D8CEBD] text-[#181314] font-semibold text-xs hover:bg-[#FAF6EE]"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleSendOffer}
                    className="py-2.5 rounded-lg bg-[#7A1C28] hover:bg-[#63141E] text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-sm"
                  >
