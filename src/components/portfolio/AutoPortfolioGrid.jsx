...
export default function AutoPortfolioGrid({ items, creatorName }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white border border-[#E8DEC8] rounded-xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {categories.map((cat) => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${selectedCategory === cat
      ...
      {filteredItems.length === 0 ? (<div className="bg-white border border-[#E8DEC8] rounded-xl p-12 text-center">
          ...
          {filteredItems.map((item) => (<div key={item.id} className="bg-white border border-[#E8DEC8] rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col justify-between">
              <div>
                {/* Thumbnail / Video Preview Area */}
                <div className="relative aspect-[16/10] bg-zinc-900 overflow-hidden cursor-pointer" onClick={() => setActiveModalItem(item)}>
                  <img src={item.thumbnailUrl} alt={item.brandName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-90"/>
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between pb-3 border-b border-[#F0E8D8]">
                    <div className="flex items-center gap-2.5">
                      <img src={item.brandLogo} alt={item.brandName} className="w-7 h-7 rounded-full object-cover border border-[#E8DEC8]"/>
                      <div>
                  ...
                  {item.clientReview && (<div className="mt-3.5 p-3 rounded-lg bg-[#FAF6EE] border border-[#E8DEC8]">
                      <div className="flex items-center gap-1 text-amber-500 mb-1">
                        {[...Array(item.clientReview.rating)].map((_, i) => (<Star key={i} className="w-3 h-3 fill-current"/>))}
                      </div>
              ...
              <div className="px-5 py-3 bg-[#FAF6EE] border-t border-[#E8DEC8] flex items-center justify-between text-[11px]">
                ...
                <button onClick={() => setActiveModalItem(item)} className="font-bold text-[#7A1C28] hover:underline">
                  View Case Study →
                </button>
              </div>
      ...
      {/* Video Modal Player */}
      {activeModalItem && (<div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full overflow-hidden shadow-2xl border border-[#E8DEC8]">
            <div className="p-4 border-b border-[#E8DEC8] flex items-center justify-between">
              <div className="flex items-center gap-2">
                <img src={activeModalItem.brandLogo} alt={activeModalItem.brandName} className="w-6 h-6 rounded-full object-cover"/>
                <h3 className="text-sm font-bold text-[#181314]">
                  {activeModalItem.brandName} — {activeModalItem.format}
                </h3>
              </div>
              <button onClick={() => setActiveModalItem(null)} className="w-8 h-8 rounded-full bg-[#FAF6EE] text-[#181314] font-bold flex items-center justify-center hover:bg-[#F2EAE0]">
                ✕
              </button>
            </div>

            <div className="aspect-video bg-black">
              <video src={activeModalItem.deliverableUrl} controls autoPlay className="w-full h-full object-contain"/>
            </div>
            ...
            <div className="p-5 bg-[#FAF6EE] border-t border-[#E8DEC8]">
              ...
              {activeModalItem.clientReview && (<p className="text-xs text-[#181314] italic">
