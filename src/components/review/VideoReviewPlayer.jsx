...
import { Play, Pause, Check, Plus, Clock, FileVideo, ShieldCheck } from "lucide-react";
export default function VideoReviewPlayer({ drafts, campaignTitle, onApproveFinal }) {
    var _a;
    const [activeVersionIndex, setActiveVersionIndex] = useState(0);
    const [isPlaying, setIsPlaying] = useState(false);
    const [currentTime, setCurrentTime] = useState(0);
    const [duration, setDuration] = useState(30);
    const [pins, setPins] = useState(((_a = drafts[0]) === null || _a === void 0 ? void 0 : _a.pins) || []);
    ...
    const togglePinResolved = (pinId) => {
        setPins(pins.map((p) => (p.id === pinId ? Object.assign(Object.assign({}, p), { resolved: !p.resolved }) : p)));
    };
    const formatSec = (sec) => {
    ...
    return (<div className="bg-white border border-[#E8DEC8] rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="p-5 border-b border-[#E8DEC8] flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#FAF6EE]">
        ...
        <div className="flex items-center gap-1.5 p-1 rounded-lg bg-white border border-[#E8DEC8]">
          {drafts.map((d, index) => (<button key={d.id} onClick={() => {
                setActiveVersionIndex(index);
                setPins(d.pins);
            }} className={`px-3 py-1 rounded text-xs font-semibold font-mono transition-all ${activeVersionIndex === index
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
        ...
        <div className="lg:col-span-7 bg-black p-4 flex flex-col justify-between">
          <div className="relative aspect-[9/16] max-h-[480px] mx-auto bg-zinc-950 rounded-xl overflow-hidden group">
            <video ref={videoRef} src={currentDraft.videoUrl} onTimeUpdate={handleTimeUpdate} onClick={togglePlay} className="w-full h-full object-contain cursor-pointer"/>

            {/* Floating Play/Pause Indicator */}
            {!isPlaying && (<div onClick={togglePlay} className="absolute inset-0 flex items-center justify-center bg-black/30 cursor-pointer">
          ...
          <div className="mt-4 space-y-2 text-white">
            ...
            <div className="relative h-4 flex items-center cursor-pointer">
              ...
              <div className="w-full h-2 rounded-full bg-zinc-800 overflow-hidden relative">
                <div className="h-full bg-[#7A1C28] transition-all" style={{ width: `${(currentTime / (duration || 1)) * 100}%` }}></div>
              </div>

              {/* Pin Markers on the Track */}
              {pins.map((pin) => (<button key={pin.id} onClick={(e) => {
                e.stopPropagation();
                jumpToTimestamp(pin.timestampSec);
            }} title={`Jump to ${formatSec(pin.timestampSec)}: ${pin.comment}`} style={{ left: `${(pin.timestampSec / (duration || 1)) * 100}%` }} className={`absolute w-3.5 h-3.5 -top-0.5 -ml-1.5 rounded-full border-2 border-white shadow-md transition-transform hover:scale-150 ${pin.resolved ? "bg-emerald-500" : "bg-amber-400 animate-pulse"}`}/>))}
            </div>

            {/* Playback Controls & Timestamp */}
            <div className="flex items-center justify-between text-xs font-mono">
              <div className="flex items-center gap-3">
                <button onClick={togglePlay} className="p-1 rounded bg-zinc-800 hover:bg-zinc-700 text-white">
        ...
        <div className="lg:col-span-5 p-5 bg-white border-t lg:border-t-0 lg:border-l border-[#E8DEC8] flex flex-col justify-between space-y-4">
          ...
          <div className="space-y-4">
            ...
            {/* Add Pin Form */}
            {showAddComment && (<form onSubmit={handleAddPin} className="p-3.5 rounded-xl bg-[#FAF6EE] border border-[#E8DEC8] space-y-2.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-[#7A1C28]">
                    Pin at timestamp: {formatSec(currentTime)}
                  </span>
                  <button type="button" onClick={() => setShowAddComment(false)} className="text-xs text-[#6C635B] hover:text-black">
                    ✕
                  </button>
                </div>
                <textarea value={newCommentText} onChange={(e) => setNewCommentText(e.target.value)} placeholder="e.g. Please brighten product label and adjust music volume..." rows={2} className="w-full text-xs p-2 rounded border border-[#E8DEC8] bg-white text-[#181314] focus:outline-none" required/>
                <div className="flex justify-end gap-2">
                  <button type="button" onClick={() => setShowAddComment(false)} className="text-xs px-3 py-1 rounded border border-[#D8CEBD] text-[#6C635B]">
                    Cancel
                  </button>
                  <button type="submit" className="text-xs px-3 py-1 rounded bg-[#7A1C28] text-white font-bold">
                    Save Pin
                  </button>
                </div>
              </form>)}
            ...
            <div className="space-y-2.5 max-h-[340px] overflow-y-auto pr-1">
              {pins.map((pin) => (<div key={pin.id} className={`p-3.5 rounded-xl border transition-all ${pin.resolved
                ? "bg-[#FAF6EE]/60 border-[#E8DEC8] opacity-75"
                : "bg-[#FAF6EE] border-[#E8DEC8]"}`}>
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <button onClick={() => jumpToTimestamp(pin.timestampSec)} className="px-2 py-0.5 rounded font-mono text-[10px] font-bold bg-[#7A1C28] text-white hover:bg-[#63141E] flex items-center gap-1">
                      <Clock className="w-3 h-3"/>
                      <span>{formatSec(pin.timestampSec)}</span>
                    </button>

                    <button onClick={() => togglePinResolved(pin.id)} className={`text-[11px] font-semibold flex items-center gap-1 px-2 py-0.5 rounded border transition ${pin.resolved
          ...
          <div className="pt-4 border-t border-[#F0E8D8] space-y-2">
            <button onClick={onApproveFinal} className="w-full py-3 rounded-xl bg-[#166534] hover:bg-[#14532D] text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-2 transform active:scale-95">
